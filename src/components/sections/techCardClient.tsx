...
export function TechCardClient({ tech, index }: TechCardClientProps) {
  ...
  return (
    <motion.div variants={fadeInUp} className="group relative">
      <div
        ref={cardRef}
        className="relative p-10 h-full flex flex-col items-start justify-between min-h-55 bg-background dark:bg-zinc-900/10 border border-border dark:border-zinc-800/50 hover:border-blue-600 dark:hover:border-blue-500/40 transition-all duration-500 overflow-hidden"
      >
        ...
        <div className="absolute inset-0 bg-linear-to-b from-blue-600/3 dark:from-blue-500/2 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
    </motion.div>
  );
}
